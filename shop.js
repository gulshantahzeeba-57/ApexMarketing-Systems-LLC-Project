        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            const closeDrawerBtn = document.getElementById("cartClose");
            ...
            /* =========================================================
               CART LOGIC — wires up "Add to Cart" buttons on product
               cards to the drawer, since this was previously missing
               ========================================================= */
            function loadCart() {
                try {
            ...
            let cart = loadCart(); // { id, title, price }
            ...
            function renderCart() {
                ...
                cart.forEach(item => {
                    ...
                    line.innerHTML = `
                        <div class="cart-line-icon"><i data-lucide="package"></i></div>
                ...
                cartItemsList.querySelectorAll(".cart-line-remove").forEach(btn => {
                    btn.addEventListener("click", () => {
                        ...
                        saveCart(cart);
                        document.querySelector(`.add-to-cart-action[data-id="${id}"]`)?.classList.remove("added");
                        renderCart();
                    });
                });
            ...
            /* =========================================================
               CMS CUSTOM PRODUCTS — rendered from the admin panel
               ========================================================= */
            (function renderCustomProducts() {
                const grid = document.querySelector(".store-cards-grid");
                if (!grid) return;
                let items = [];
                try {
                ...
                items.forEach((item) => {
                    ...
                    card.className = "store-product-card-interactive";
                    card.innerHTML = `
                        <div class="card-trolley-watermark">
                            <svg width="110" height="110" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="9" cy="21" r="1"></circle><circle cx="20" cy="21" r="1"></circle><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"></path></svg>
                        </div>
                        ...
                        <div class="card-action-footer">
                            <div class="card-display-price">$${item.price}</div>
            ...
            document.querySelectorAll(".add-to-cart-action").forEach((btn, index) => {
                const card = btn.closest(".store-product-card-interactive");
                const title = card?.querySelector(".card-item-title")?.textContent.trim() || "Product";
                const priceText = card?.querySelector(".card-display-price")?.textContent.trim() || "$0";
