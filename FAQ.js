document.addEventListener("DOMContentLoaded", () => {
  ...
  const revealSelector = [
    ".faq-container h2", ".faq-subtitle", ".accordion-item", ".footer-col"
  ].join(", ");
  ...
  groups.forEach(list => {
    list.forEach((el, i) => { el.style.transitionDelay = `${Math.min(i, 6) * 90}ms`; });
  });
  ...
  /* =========================================================
     CART DRAWER — persisted across every page via localStorage
     ========================================================= */
  function loadCart() {
    try {
  ...
  accordionHeaders.forEach(header => {
    header.addEventListener("click", function () {
      ...
      // Pehle se khule hue baki sab FAQs ko close karne ke liye
      document.querySelectorAll(".accordion-item").forEach(item => {
        item.classList.remove("active");
        item.querySelector(".accordion-content").style.maxHeight = null;
      });

      // Agar click kiya hua pehle se active nahi tha, to usey open kar do
      if (!isActive) {
