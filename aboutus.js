document.addEventListener("DOMContentLoaded", () => {
  ...
  /* =========================================================
     CMS CUSTOM TEAM — rendered from the admin panel
     ========================================================= */
  (function renderCustomTeam() {
    const grid = document.getElementById("leadershipGrid");
    if (!grid) return;
    let items = [];
    try {
      const raw = localStorage.getItem("cms_team");
    ...
    items.forEach((item) => {
      ...
      card.innerHTML = `
        <div class="card-header">
          ...
          <span class="profile-role">${item.role}</span>
        </div>
        <div class="inner-visual-box">
          <div class="initials-badge">${item.initials}</div>
        </div>
        <p class="profile-desc">${item.description}</p>
      `;
  ...
  /* =========================================================
     PROFILE CARD — 3D tilt following the mouse (every card)
     ========================================================= */
  document.querySelectorAll(".profile-card").forEach((tiltCard) => {
    const maxTilt = 10;
